...
def bin_to_dec(binary_string):
    ...
    for i in range(0,len(binary_string)):
        list_of_exponents.append(2**i)
    new_exponent_list = list_of_exponents[::-1]
    for char in binary_string:
        converted_binary.append(int(char)*new_exponent_list[index])
        index += 1
    return sum(converted_binary)
...
def telephone_cipher(message):
    ...
    for char in message:
        if char in encoder_dict:
            key_sequence = encoder_dict[char]
            if previous_key and key_sequence[0] == previous_key:
                encoded_message.append('_')
            encoded_message.append(key_sequence)
            previous_key = key_sequence[0]
    ...
    for part in parts:
        ...
        while i < len(part):
            for length in range(4, 0, -1):
                if part[i:i+length] in decipher_dict:
                    decoded_message.append(decipher_dict[part[i:i+length]])
                    i += length
                    break
