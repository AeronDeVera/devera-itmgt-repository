...
#### Convert punctuation symbols into a whitespace
unnecessary_punctuation = r"&$@[].,'#()-\"!?’_;:/"

word_list = []
for i in range(0,len(jsondata)):
    processed_line=str(jsondata.get(str(i))).upper().replace("<P>","").replace("</P>","").replace("<B>","").replace("</B>","").replace("<I>","").replace("</I>","").replace("\n","").replace("&"," ").replace("$"," ").replace("@"," ").replace("["," ").replace("]"," ").replace("."," ").replace(","," ").replace('"',' ').replace("#"," ").replace("("," ").replace(")"," ").replace("-"," ").replace("\"," ").replace("""," ").replace("!"," ").replace("?"," ").replace("’"," ").replace("_"," ").replace(";"," ").replace(":"," ").replace("/"," ").replace("'"," ")
...
for i in range(0,len(joined_list.split(" "))):
    while len(joined_list.split(" ")[i]) >= 3:
        wordcount_dictionary[str(joined_list.split(" ")[i])]=joined_list.count(str(joined_list.split(" ")[i]))
...
final_wordlist_of_tuples=wordcount_dictionary.items()
...
max_z = np.array([10, 12, 16])
gold_lumber_food_constraints = np.array([[100,600,750],[500,750,300],[50,45,75]])

print(max_z)
print(gold_lumber_food_constraints)
...
def Collatz(start_number):
    ...
    while start_number != 1:
        ...
        if start_number % 2==0:
            ...
            collatz_sequence.append(int(start_number))
    ...
    collatz_dictionary = {}
    collatz_dictionary["collatz_sequence"]=collatz_sequence
    collatz_dictionary["collatz_length"]=collatz_length
    collatz_dictionary["max_collatz"]=collatz_sequence
    collatz_dictionary["sequence_sum"]=sequence_sum
...
def Collatz_winner(number_list):
    ...
    for i in range(0,len(number_list)):
        max_collatz_list.append(Max_Collatz(number_list[i]))
    for i in range(0,len(number_list)):
        collatz_length_list.append(Collatz_Length(number_list[i]))

    if max_collatz_list.index(max(max_collatz_list)) == collatz_length_list.index(max(collatz_length_list)):
        return max(max_collatz_list)
    else:
        return None
